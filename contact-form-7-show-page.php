 ...
 *
 * @param  Array $panels Array form editor panel args.
 *
 * @return Array $panels  Array form editor panel args.
 */
...
function wpcf7sp_editor_panel_cf7_show_page( $post ) { ?>
    <div class="wpcf7sp_panel">
    	...
    	if ( $post_query->have_posts() ) { ?>
            ...
            <small><?php _e( 'No posts or pages use this form right now.', 'wpcf7sp' ); ?></small>
    	<?php } ?>
        ...
        // var_dump( $wpcf7sp_widget_results );

    	/******************************* if case should be applied right here **********************************************
    	* because no need to get zero result
    	* and unserialize it
    	* loop it
    	* and regex it for $widget_ids
    	* and again query for sidebar
    	* and again loop sidebar results to check $widget_ids
    	********************************************************************************************************************/
        if( !empty( $wpcf7sp_widget_results ) ) {
        	...
        	$widget_resp_sidebar_query = $wpdb->prepare( "SELECT * FROM `$wpcf7sp_table` WHERE `option_name` = 'sidebars_widgets'", $wpcf7sp_search_string );
            ...
            <!-- <h4><?php //_e( 'Lists of Text widgets using this form.', 'wpcf7sp' ); ?></h4> -->
            <?php
            if ( ! is_null( $wpcf7sp_text_widgets ) ): ?>
                ...
                /******************************* if case should end here **********************************************/
                ?>
                <!-- <small><?php //_e( 'No Text widgets uses this form right now.', 'wpcf7sp' ); ?></small> -->
        	<?php endif;
        } else { ?>
            <small><?php _e( 'No Text widgets uses this form right now.', 'wpcf7sp' ); ?></small>
    <?php
